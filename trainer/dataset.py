...
class Batch_Balanced_Dataset(object):
    ...
    def __init__(self, opt):
        ...
        """
        log = open(f'./saved_models/test/log_dataset.txt', 'a')
        dashed_line = '-' * 80
        print(dashed_line)
...
class OCRDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        if self.opt.data_filtering_off:
            ...
            for index in range(self.nSamples):
                if len(self.df.at[index, 'filename'].split(';')) > 1:
                    label = self.df.at[index, 'filename'].split(';')[1]
                    try:
    ...
    def __getitem__(self, index):
        ...
        obj = img_fname.split(';')
        if len(obj) > 1:
            img_fpath = os.path.join(self.root, obj[0])
            label = obj[1]
            if self.opt.rgb:
